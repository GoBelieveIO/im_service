...
func GetRelationship(appid int64, uid int64, friend_uid int64) (Relationship, error) {
	conn := redis_pool.Get()
	defer conn.Close()
	...
	err := conn.Send("MULTI")
	if err != nil {
		log.Info("send err:", err)
		return rs, err
	}

	key := fmt.Sprintf("friends_%d_%d", appid, uid)	
	err = conn.Send("SISMEMBER", key, friend_uid)
	if err != nil {
		log.Info("send err:", err)
		return rs, err
	}

	key = fmt.Sprintf("friends_%d_%d", appid, friend_uid)
	err = conn.Send("SISMEMBER", key, uid)
	if err != nil {
		log.Info("send err:", err)
		return rs, err
	}

	key = fmt.Sprintf("blacklist_%d_%d", appid, friend_uid)
	err = conn.Send("SISMEMBER", key, uid)
	if err != nil {
		log.Info("send err:", err)
		return rs, err
	...
	key = fmt.Sprintf("blacklist_%d_%d", appid, uid)
	err = conn.Send("SISMEMBER", key, friend_uid)
	if err != nil {
		log.Info("send err:", err)
		return rs, err
	...
	_, err = redis.Scan(reply, &is_my_friend, &is_your_friend, &is_in_my_blacklist, &is_in_your_blacklist)
	if err != nil {
		log.Info("scan err:", err)
		return rs, err
	}
